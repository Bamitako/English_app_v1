...
1. https://www.pexels.com/api/ で無料APIキーを取得する（メール登録のみ・審査なし）
2. リポジトリのルートで実行する
...
50語ぶんの写真がこのフォルダに落ちます。検索ワードは `src/data/words.ts` の
`query` に単語ごとに書いてあります（`borrow` → "handing over a pen" のように、
単語そのままではなく場面で引いています）。

すでに画像がある単語はスキップされるので、途中で止めても再実行すれば続きから進みます。
特定の単語だけ取り直したいときは:

```bash
PEXELS_API_KEY=... npm run fetch-images -- --force sour barely
```

取得できなかった単語は最後にまとめて表示されます。`query` を書き換えて
`--force` で引き直してください。

**取得後は必ず一通り目視で確認してください。** 検索で1枚目を機械的に取るので、
意図と違う写真が混じります。特に `barely` `on-purpose` `by-accident`
`all-of-a-sudden` のような抽象語は、まともな写真が存在しないことが多いです。
その場合は無理に写真を当てず、ファイルを削除して絵文字＋シチュエーション文の
表示に戻すほうが学習体験としては良いです。
